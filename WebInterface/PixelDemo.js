...
function setMinQP() {
	let qp = document.getElementById("MinQP").value;
    let descriptor = {
        MinQP: qp
    }
    emitUIInteraction(descriptor);
...
function setMaxQP() {
	let qp = document.getElementById("MaxQP").value;
    let descriptor = {
        MaxQP: qp
    }
...
function setMaxBitrate() {
	let mbps = document.getElementById("MaxBitrate").value;
    let descriptor = {
        MaxBitrate: mbps
    }
    emitUIInteraction(descriptor);
...
function setFramerateCap() {
	let cap = document.getElementById("MaxFPS").value;
    let descriptor = {
        MaxFPS: cap
    }
    emitUIInteraction(descriptor);
...
function onParagonLoad() {
	...
	let audioCheck = document.getElementById('ck-audio');
	if(audioCheck){
		audioCheck.onclick = function(){
			if (!isMuted) {
				enabledAudio();
			} else {
				disableAudio();
...
function onAudioChange()
{
	...
	if(audioImg){
		audioImg.src = isMuted ? 'images/Muted.png' : 'images/Unmuted.png'
		audioImg.alt = isMuted ? 'Shrink to normal size' : 'Maximise to Fullscreen'
	}
}
